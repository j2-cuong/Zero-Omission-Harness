...
class StateValidator:
    ...
    def _default_state_machine(self) -> Dict:
        ...
        return {
            'states': {
                'interview': {'allowed_transitions': ['planning'], 'description': 'Initial interview'},
                'planning': {'allowed_transitions': ['coding', 'interview'], 'description': 'Planning phase'},
                'coding': {'allowed_transitions': ['testing', 'planning'], 'description': 'Coding phase'},
                'testing': {'allowed_transitions': ['release', 'coding'], 'description': 'Testing phase'},
                'release': {'allowed_transitions': ['interview'], 'description': 'Release phase'}
            }
        }
    ...
    def validate_transition(self, to_phase: str) -> Dict[str, Any]:
        ...
        states_config = self.state_machine.get('states', {})
        state_config = states_config.get(current_phase, {})
        valid_next = state_config.get('allowed_transitions', [])
        ...
        # Get guards for the to_phase
        to_state_config = states_config.get(to_phase, {})
        guards = to_state_config.get('guards', [])
        failed_guards = []
...
"""
    ...
    def get_allowed_transitions(self) -> List[str]:
        ...
        current_phase = self.get_current_phase()
        return self.state_machine.get('states', {}).get(current_phase, {}).get('allowed_transitions', [])
